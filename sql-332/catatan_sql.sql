...
alter table nilai add constraint nilai_ujian_fkey foreign key (kode_ujian) references public.ujian(kode_ujian);
...
--case when
select nama_mahasiswa, nama_ujian, nilai,
...
--coalesce // check all parameter if null, return first value not null
select coalesce(1, 2, 3); -- return 1
select coalesce(3, null, 2, null); -- return 3
select coalesce(null, 3, 2, 1); -- return 3
...
--union
select *, 'lulus' as keterangan
from view_nilai where nilai >= 80
union all 
select *, 'tidak lulus' as keterangan
from view_nilai where nilai <= 80;
...
--postgre transaction
/* ACID
-- Atomicity	= Ensures transaction are treated as a single unit
-- Consistency	= Ensures consistency of data inputted to database
-- Isolation	= Ensures transaction written at the same time doesnt overwrite each other
-- Durability	= Ensures data commited stay commited
